...
_AU_frelatif = ArretUtilisateur


def executer_optimisation_frelatif(
    ...
    """
    Pivot F-relatif.
    Phase 1 : alignement à F_exp_max (identique à Fmax).
    Phase 2 : décalage recalculé à chaque portion × F_exp_max — c'est la
              différence fondamentale avec Fmax où le décalage est figé.
    """
    def _log(msg):
    ...
    # ── Phase 1 : Fonction_cout — alignement à F_exp_max ─────────────────────
    def Fonction_cout(params):
        x = params['x'].value
        y = params['y'].value
        point = (x, y)
        ...
        # Trouver l'indice correspondant à F_exp_max
        ...
        for i in range(len(F_num_tempo) - 1):
            if F_num_tempo[i] <= F_exp_max <= F_num_tempo[i + 1]:
                h_inter = interp1d(
                    [F_num_tempo[i], F_num_tempo[i + 1]],
                    [h_num_tempo[i], h_num_tempo[i + 1]],
                    kind='linear',
                )
                h_max = h_inter(F_exp_max)
        ...
        rayon_num = interpolation_Hencky(
            point, x1, y1, rayon_test_1, x2, y2, rayon_test_2,
            x3, y3, rayon_test_3, x4, y4, rayon_test_4,
        )
    ...
    _log("─── Phase 1 : optimisation globale (alignement F_exp_max) ───")
    params = Parameters()
    params.add('x', value=best_sig_y, vary=True, min=33.6, max=3360.0)
    params.add('y', value=best_n,     vary=True, min=0.0,  max=0.45)

    result = minimize(Fonction_cout, params, method='Nelder-Mead',
                      iter_cb=iter_cb_phase1, options={'xatol': 1e-10, 'fatol': 1e-10})
    sigma_y_optimale = result.params['x'].value
    n_optimale       = result.params['y'].value
    _log(f"  → σ_y = {sigma_y_optimale:.3f} MPa  |  n = {n_optimale:.4f}")

    # ── Phase 2 : Fonction_cout_1 — décalage recalculé à portion × F_max ─────
    #   DIFFÉRENCE FONDAMENTALE vs fmax : ici le décalage n'est PAS figé,
    #   il est recalculé pour chaque portion en alignant au niveau portion*F_max.
    def Fonction_cout_1(params, portion):
        x = params['x'].value
        y = params['y'].value
        point = (x, y)
        ...
        # Seuil = portion × F_exp_max  (différent de fmax où c'est toujours F_exp_max)
        F_exp_portion = portion * F_exp_max
        indiceF_scale = np.where(F_exp <= F_exp_portion)[0][-1]
        F_exp_scaled = F_exp[:indiceF_scale + 1]
        h_exp_scaled = h_exp[:indiceF_scale + 1]

        # h_num au niveau de F_exp_portion
        h_max = delta_h
        for i in range(len(F_num_tempo) - 1):
        ...
        # Recalcul du décalage à cette portion (≠ fmax qui utilise decalage_global figé)
        decalage = -h_exp_scaled[-1] + h_max
        h_exp_decale = h_exp_scaled + decalage
    ...
    for k_portion, portion in enumerate(portions_list, 1):
        ...
        def _r1(p, _portion=portion):
            return Fonction_cout_1(p, _portion)

        result = minimize(_r1, params, method='Nelder-Mead',
    ...
    # ── Phase 3 : Hessienne par différences finies ────────────────────────────
    ...
    for k_h, (portion, values) in enumerate(resultats_optimisation.items(), 1):
        ...
        def _r1p(p, _portion=portion):
            return Fonction_cout_1(p, _portion)

        # dérivée d²/dK²
        K_values = [K_final + dk * delta_k for dk in (-2, -1, 0, 1, 2)]
        resid_valeur_k = []
        for K in K_values:
            params['x'].value = calculate_sigma_y(K, n_p, E)
            params['y'].value = n_p
            resid_valeur_k.append(_r1p(params))

        # dérivée d²/dn²
        n_values = [n_p + dk * delta_k for dk in (-2, -1, 0, 1, 2)]
        resid_valeur_n = []
        for n in n_values:
            params['x'].value = calculate_sigma_y(K_final, n, E)
            params['y'].value = n
            resid_valeur_n.append(_r1p(params))
        ...
        # dérivée mixte d²/dKdn (16 évaluations)
        mixed_points = [
            (K_final - delta_k,       n_p - delta_k),
            (K_final + delta_k,       n_p - delta_k),
            (K_final + delta_k,       n_p + delta_k),
            (K_final - delta_k,       n_p + delta_k),
            (K_final - 2.0 * delta_k, n_p - 2.0 * delta_k),
            (K_final - delta_k,       n_p - 2.0 * delta_k),
            (K_final + delta_k,       n_p - 2.0 * delta_k),
            (K_final + 2.0 * delta_k, n_p - 2.0 * delta_k),
            (K_final + 2.0 * delta_k, n_p + delta_k),
            (K_final + 2.0 * delta_k, n_p + 2.0 * delta_k),
            (K_final + 2.0 * delta_k, n_p - delta_k),
            (K_final - 2.0 * delta_k, n_p + 2.0 * delta_k),
            (K_final - delta_k,       n_p + 2.0 * delta_k),
        ...
        f_vals = []
        for K_pt, n_pt in mixed_points:
            params['x'].value = calculate_sigma_y(K_pt, n_pt, E)
            params['y'].value = n_pt
            f_vals.append(_r1p(params))

        (f_1_1, f1_1, f11, f_11,
         f_2_2, f_1_2, f1_2, f2_2,
         f21, f22, f2_1, f_22,
         f_12, f12, f_2_1, f_21) = f_vals
    ...
    for portion, values in resultats_optimisation.items():
        ...
        residu_par_portion.append(Fonction_cout_1(params, portion))
