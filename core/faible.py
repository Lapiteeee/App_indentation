...
def executer_optimisation_faible(
    E, sig_y_guess, n_guess,
    chemin_courbe, bdd_module, portions,
    chemins_maillage,
    F_pivot,          # force de pivot (N) — point d'alignement choisi par l'utilisateur
    callback=None,
    stop_flag=None,
    log_callback=None,
):
    ...
    # Interpolation de h_exp_pivot à partir de la force pivot
    F_exp_inter = interp1d(F_exp, h_exp, kind='linear', fill_value="extrapolate")
    h_exp_pivot = float(F_exp_inter(F_pivot))
    _log(f"  Pivot : F = {F_pivot:.4f} N  →  h_exp_pivot = {h_exp_pivot:.4f} µm")

    decalage_list = []
    rayon_list = []

    # -------------------------------------------------------------------------
    # Phase 1 — rosenbrock global, alignement au pivot faible
    # -------------------------------------------------------------------------
    def rosenbrock(params):
        ...
        # Alignement au pivot faible (et non à F_max comme dans Fmax)
        h_max = delta_h
        for i in range(len(F_num_tempo) - 1):
            if F_num_tempo[i] <= F_pivot <= F_num_tempo[i + 1]:
                h_inter = interp1d(
                    [F_num_tempo[i], F_num_tempo[i + 1]],
                    [h_num_tempo[i], h_num_tempo[i + 1]],
                    kind='linear',
                )
                h_max = h_inter(F_pivot)

        decalage = -h_exp_pivot + h_max
        h_exp_decale = h_exp + decalage
        ...
        # Portage fidèle : x-axis = h_exp (non décalé), comme dans le code brut
    ...
    _log("─── Phase 1 : optimisation globale (pivot faible) ───")
    ...
    _log(f"  → σ_y = {sigma_y_optimale:.3f} MPa  |  n = {n_optimale:.4f}  |  décalage = {decalage_global:.4f}")

    # -------------------------------------------------------------------------
    # Phase 2 — rosenbrock_1 par portion, pivot figé à F_pivot
    # -------------------------------------------------------------------------
    def rosenbrock_1(params, portion):
        ...
        # Alignement toujours au pivot faible (constant entre les portions)
        h_max = delta_h
        for i in range(len(F_num_tempo) - 1):
            if F_num_tempo[i] <= F_pivot <= F_num_tempo[i + 1]:
                h_inter = interp1d(
